...
public class UserServiceTest {
	...
	public void testGetUser(){
		...
		UserDTO mockDtoUser = new UserDTO();
		mockDtoUser.setName("Thanga");
		Mockito.when(mockUserDao.getUser("1")).thenReturn(mockDtoUser);
		
		UserService userService = new UserService();
		userService.setUserDao(mockUserDao);
		UserDTO userDto = userService.getUser("1");
		Assert.assertEquals("Thanga", userDto.getName());

		//null id
		Mockito.when(mockUserDao.getUser(null)).thenThrow(new IllegalArgumentException("null id"));
		UserDTO userDtoNull = userService.getUser(null);
